...
function layout({ title, description, canonical, body, extraHead='', bodyClass='', jsonld='' }) {
  ...
  const navHtml = NAV.map(n => {
    ...
    if (n.mega) {
      ...
      const feature = f ? `<a class="mega__feature" href="${f.href}"><span class="mega__feature-eyebrow">${f.eyebrow}</span><span class="mega__feature-title">${f.title}</span><span class="mega__feature-text">${f.text}</span><span class="mega__feature-cta">${f.cta} &rarr;</span></a>` : '';
...
.mega__inner--feature{ grid-template-columns: 1fr minmax(260px, 340px); align-items: stretch; }
.mega__inner:not(.mega__inner--feature){ grid-template-columns: auto; justify-content: center; }
.mega__inner:not(.mega__inner--feature) .mega__cols{ min-inline-size: min(680px, 84vw); }
.mega__cols{ display: grid; grid-template-columns: repeat(2, minmax(180px, 1fr)); gap: clamp(1.5rem, 4vw, 3.5rem); }
.mega__col{ display: grid; gap: 0.15rem; align-content: start; }
.mega__title{
  font-family: 'Montserrat', sans-serif;
  text-transform: uppercase;
...
.mega__col a:hover{ background: var(--vellum); color: var(--teal); transform: translateX(3px); }
.mega__feature{
...
.mega__feature-title{ font-family: 'Montserrat', system-ui, sans-serif; font-size: 1.18rem; line-height: 1.25; }
.mega__feature-text{ font-size: 0.9rem; opacity: 0.85; line-height: 1.5; }
.mega__feature-cta{ margin-block-start: 0.4rem; font-size: 0.85rem; font-weight: 500; color: var(--cream); }
...
.hm-hero__h1{
  color: var(--cream);
  font-size: clamp(2.35rem, 8.5vw, 7.4rem);
  line-height: 0.99;
  letter-spacing: -0.04em;
  font-weight: 800;
  text-shadow: 0 1px 24px oklch(20% 0.005 250 / 0.45);
}
.hm-hero__h1 em{ font-style: normal; color: var(--sand); }
